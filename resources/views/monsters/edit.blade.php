...
<div class="flex bg-gray-900 min-h-screen">
        ...
        <div class="flex-1 ml-72 p-8">
            <div class="mb-6 flex items-center justify-between">
                <h2 class="text-3xl font-bold text-white">Edit Monster: {{ $monster->name }}</h2>
                ...
                <form action="{{ route('monsters.update', $monster) }}" method="POST" enctype="multipart/form-data">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        <div>
                            ...
                            <input type="text" name="name" id="name" value="{{ old('name', $monster->name) }}" class="w-full bg-gray-700 border border-gray-600 rounded-lg px-4 py-2 text-white focus:outline-none focus:ring-2 focus:ring-yellow-500" required>
                            @error('name')
                        ...
                        <div class="md:col-span-2">
                            ...
                            <textarea name="description" id="description" rows="4" class="w-full bg-gray-700 border border-gray-600 rounded-lg px-4 py-2 text-white focus:outline-none focus:ring-2 focus:ring-yellow-500" required>{{ old('description', $monster->description) }}</textarea>
                            @error('description')
                                <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
                            @enderror
                        </div>
                        ...
                        <div>
                            ...
                            <select name="danger_level" id="danger_level" class="w-full bg-gray-700 border border-gray-600 rounded-lg px-4 py-2 text-white focus:outline-none focus:ring-2 focus:ring-yellow-500" required>
                                ...
                                <option value="Medium" {{ old('danger_level', $monster->danger_level) == 'Medium' ? 'selected' : '' }}>Medium</option>
                                ...
                                <option value="Warning" {{ old('danger_level', $monster->danger_level) == 'Warning' ? 'selected' : '' }}>Warning</option>
                            </select>
                            @error('danger_level')
                                <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
                            @enderror
                        ...
                        <div>
                            ...
                            <input type="number" name="sightings" id="sightings" value="{{ old('sightings', $monster->sightings) }}" class="w-full bg-gray-700 border border-gray-600 rounded-lg px-4 py-2 text-white focus:outline-none focus:ring-2 focus:ring-yellow-500" required min="0">
                            @error('sightings')
                                <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
                            @enderror
                        </div>
                        ...
                        <div>
                            ...
                            <input type="number" name="lat" id="lat" value="{{ old('lat', $monster->lat) }}" step="any" class="w-full bg-gray-700 border border-gray-600 rounded-lg px-4 py-2 text-white focus:outline-none focus:ring-2 focus:ring-yellow-500" required>
                            @error('lat')
                                <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
                            @enderror
                        </div>
                        ...
                        <div>
                            ...
                            <input type="number" name="lng" id="lng" value="{{ old('lng', $monster->lng) }}" step="any" class="w-full bg-gray-700 border border-gray-600 rounded-lg px-4 py-2 text-white focus:outline-none focus:ring-2 focus:ring-yellow-500" required>
                            @error('lng')
                                <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
                            @enderror
                        </div>
    ...
    <img id="preview-image" src="{{ $monster->image_path ? route('monsters.image', $monster) : '#' }}" alt="Preview" class="mx-auto max-h-48 rounded">
</div>
                                    <div id="upload-prompt" class="{{ $monster->image_path ? 'hidden' : '' }} flex flex-col items-center justify-center py-6">
                                    ...
                                    <div class="{{ $monster->image_path ? '' : 'hidden' }} mt-4">
                                        <p class="text-gray-400">Click to change image</p>
                                    </div>
                                </label>
                            </div>
                            @error('image')
                    ...
                    <div class="mt-8 flex justify-end space-x-4">
                        ...
                        <button type="submit" class="px-6 py-3 bg-yellow-600 text-white rounded-lg hover:bg-yellow-700">Update Monster</button>
                    </div>
                </form>
            </div>
        </div>
