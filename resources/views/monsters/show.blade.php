...
@else
                    ...
                    <div class="p-8">
                        <div class="flex items-center space-x-2 mb-4">
                            ...
                            <span class="
                                @if($monster->danger_level == 'Low') bg-green-900 text-green-300
                                @elseif($monster->danger_level == 'Medium') bg-yellow-900 text-yellow-300
                                @elseif($monster->danger_level == 'High') bg-orange-900 text-orange-300
                                @elseif($monster->danger_level == 'Extreme') bg-red-900 text-red-300
                                @elseif($monster->danger_level == 'Warning') bg-purple-900 text-purple-300
                                @else bg-gray-900 text-gray-300
                                @endif
                                px-3 py-1 rounded-full text-sm font-medium">
                                Danger: {{ $monster->danger_level }}
                            </span>
                        </div>
                        
                        <div class="text-gray-400 mb-6">
                            <p class="text-lg">{{ $monster->description }}</p>
                        </div>
                        
                        <div class="border-t border-gray-700 pt-6">
                            <h3 class="text-xl font-bold text-white mb-4">Monster Stats</h3>
                            
                            <div class="grid grid-cols-2 gap-4">
                                <div class="bg-gray-700 p-4 rounded-lg">
                                    <div class="text-gray-400 text-sm">Reported Sightings</div>
                                    <div class="text-2xl font-bold text-white">{{ $monster->sightings }}</div>
                                </div>
                                
                                <div class="bg-gray-700 p-4 rounded-lg">
                                    <div class="text-gray-400 text-sm">Last Updated</div>
                                    <div class="text-lg font-bold text-white">{{ $monster->updated_at->format('M d, Y') }}</div>
                                </div>
                            </div>
                        </div>
                        ...
                        <div class="mt-8 flex space-x-4">
                            @if(auth()->user()->is_admin)
                                <a href="{{ route('admin.monsters.edit', $monster) }}" class="px-4 py-2 bg-blue-600 text-white rounded-lg hover:bg-blue-700 flex items-center gap-2">
                                    <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                                ...
                                <form action="{{ route('admin.monsters.destroy', $monster) }}" method="POST" onsubmit="return confirm('Are you sure you want to delete this monster?');" class="inline">
                                    @csrf
                                    @method('DELETE')
